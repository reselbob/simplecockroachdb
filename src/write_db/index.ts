...
[DataTypes.postgres.INTEGER, DataTypes.postgres.BIGINT].forEach(function (intType) {
    ...
    intType.prototype.$stringify = intType.prototype._stringify = function stringify(value: any) {
        const rep = String(value);
        if (!/^[-+]?[0-9]+$/.test(rep)) {
            throw new Sequelize.ValidationError(`${value} is not a valid integer`);
        }
        return rep;
    }
});
...
ConnectionManager.prototype._loadDialectModule = function (...args: any) {
    const pg = this.__loadDialectModule(...args);
    pg.types.setTypeParser(20, function (val: any) {
...
QueryGenerator.prototype.describeTableQuery = function (...args: any) {
...
Model.findByPk = async function findByPk(param: any, options : any) {
